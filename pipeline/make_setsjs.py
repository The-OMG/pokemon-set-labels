...
os.makedirs('app/img/logos', exist_ok=True)
out = []
for s in sets:
    ...
    if logo and os.path.exists(logo):
        dest = f'app/img/logos/{s["slug"]}.png'; shutil.copyfile(logo, dest); rec['logo_img'] = f'img/logos/{s["slug"]}.png'
    out.append(rec)
...
open('app/sets.js', 'w', encoding='utf-8').write(js)
...
tot = sum(os.path.getsize(os.path.join('app/img/logos', f)) for f in os.listdir('app/img/logos')); print('logos dir', round(tot/1e6, 1), 'MB', len(os.listdir('app/img/logos')), 'files')
